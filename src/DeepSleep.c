...
void touchpad_test(){
    uint16_t raw_val;
    touch_pad_read(TOUCH_PAD_NUM2, &raw_val);

    // Get the currently configured threshold
    uint16_t threshold;
    touch_pad_get_thresh(TOUCH_PAD_NUM2, &threshold);

    printf("Touch pad %d: raw value = %d, threshold = %d -> ", TOUCH_PAD_NUM2, raw_val, threshold);

    if (raw_val < threshold) {
        printf("PAD IS TOUCHED!\n");
    } else {
        printf("Pad not touched.\n");
    }
}
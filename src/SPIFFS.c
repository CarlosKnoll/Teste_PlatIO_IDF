...
static void init_web_page_buffer(char* PATH)
{
    FILE *fp = fopen(PATH, "rb");
    if (!fp) {
        ESP_LOGE(SPIFFS_TAG, "File not found: %s", PATH);
        return;
    }

    size_t n = fread(index_html, 1, sizeof(index_html)-1, fp);
    fclose(fp);
    index_html[n] = '\0';  // null-terminate

    // Parse % characters safely into final_html
    size_t i = 0, j = 0;
    while (i < n && j < sizeof(final_html)-2) {
        if (index_html[i] == '%') {
            final_html[j++] = '%';
        }
...
{
    init_web_page_buffer(PATH);

    if (strlen(final_html) == 0) {
        return httpd_resp_send_500(req); // nothing to send
    }

    return httpd_resp_send(req, final_html, HTTPD_RESP_USE_STRLEN);
}